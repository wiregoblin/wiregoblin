...
// operator: lt
// expected: "3"
// target_step_uid: step-id-or-key
// wait_seconds: 5
...
func decodeConfig(step models.Step) gotoConfig {
	config := gotoConfig{}
	...
	if value, ok := step.Config["target_step_uid"].(string); ok {
		config.TargetStepUID = value
	}
