...
func run(projectPath, workflowName string, verbosity int, jsonOutput bool, stdout, stderr io.Writer) error {
	...
	return app.ExecuteWorkflow(workflowName, cliapp.ExecuteOptions{
...
Usage:
  wiregoblin run [flags] <workflow>

Examples:
  wiregoblin run http_example
  wiregoblin run -p config/myproject.yaml http_example
  wiregoblin run -v -p config/myproject.yaml http_example
  wiregoblin run -vv -p config/myproject.yaml http_example
  wiregoblin run -vvv -p config/myproject.yaml http_example
  wiregoblin run --json -p config/myproject.yaml http_example
`

const runUsageText = `Run a workflow from a project config

Usage:
  wiregoblin run [flags] <workflow_id>
